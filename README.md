...
- Desktop-style local launcher that starts the built frontend, FastAPI backend, and an app-mode browser window from a double-click entry.
...
5. Keep the lightweight desktop launcher for local demos, but defer full packaged desktop/system-audio capture until the browser workflow has measurable stability. At that point, evaluate Tauri or Electron against real capture, packaging, and memory requirements.
...
4. Opens Microsoft Edge or Chrome in app mode, which removes the normal browser chrome.
5. Stops the services it started when the app window exits.

Startup logs are written to `logs/desktop-launcher.log`. Override paths or ports with `AI_INTERPRETER_PYTHON`, `AI_INTERPRETER_BROWSER`, `AI_INTERPRETER_BACKEND_PORT`, or `AI_INTERPRETER_FRONTEND_PORT` when needed.
