...
"""
        ...
        for segment in revisable:
            ...
            if cached_translation is not None:
                if self._should_revise(segment.text_translated, cached_translation):
                    revisions.append(self._build_translation_revision(segment, cached_translation))
                continue
            ...
            if self._should_revise(segment.text_translated, new_translation):
                revisions.append(self._build_translation_revision(segment, new_translation))
    ...
    async def check_asr_correction(
        ...
        corrected = (await nmt.complete_text(prompt, system_prompt=self.ASR_CORRECTION_PROMPT)).strip()

        if not corrected or corrected == "CORRECT":
            return None

        if not self._should_revise(segment.text_asr, corrected):
            return None
    ...
    def _build_translation_revision(self, segment: Segment, new_text: str) -> RevisionResult:
        logger.info(
